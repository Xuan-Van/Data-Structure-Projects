Status Query()//航线查询
{
    while(1)
    {
        ...
        printf("请输入目的地:");
        scanf("%s",place2);
        //printf("出发\t抵达\t航班号\t航空公司\t起飞日期\t起飞时间\t到达时间\t头等舱\t商务舱\t经济舱\t起步价\t余票量\t星期\t\n"); 
        for(i=0;i<airnum;i++)
        {
...
Status Booking(char place1[15],char place2[15])//订票
{
    ...
    while(1)
    {
        ...
        printf("请输入日期(00-00):");
        scanf("%d-%d",&m,&n);
        printf("序号\t出发\t抵达\t航班号\t航空公司\t起飞日期\t起飞时间\t到达时间\t头等舱\t商务舱\t经济舱\t起步价\t余票量\t星期\t\n");
        for(i=0;i<airnum;i++)
        {
    ...
    while(count)
    {
        ...
        if(flag==1)
        {
            while(1)
            {
                ...
                {
                    printf("身份证号为%s的顾客已下单本次航班,是否重新录入顾客信息(y|n):",C.ID);
                    scanf(" %c",&choice);
                    if(choice=='y') continue;
                    else break;
                }
...
Status Conflict(char id[],Airline *B)//判断订票时间是否冲突
{
    ...
    for(i=0;i<airnum;i++)
    {
        if(Search(A[i].L,id,&E))
        {
            if(DayConflit(B,&A[i]))
            {
                printf("出发:%s\t抵达:%s\t起飞日期:%02d月%02d日\t起飞时间:%02d:%02d\t到达时间:%02d:%02d\t",A[i].departure,A[i].destination,A[i].month,A[i].day,A[i].hour,A[i].min,A[i].nexthour,A[i].nextmin);
                return ERROR;
            }
        }
    }
