Status Information()//显示信息
{
    ...
    printf("请输入您的身份证号:");
    scanf("%s",id);
    printf("序号\t起飞机场\t降落机场\t出发\t抵达\t航班号\t飞机号\t起飞日期\t起飞时间\t到达时间\t舱位\t价格\t星期\n");
    for(i=0;i<airnum;i++)
    {
        if(Search(A[i].L,id,&C))
        {
            printf("%d\t%s\t%s\t%s\t%s\t%s\t%s\t%02d月%02d号\t%02d:%02d\t\t%02d:%02d\t\t",A[i].number,A[i].airport,A[i].nextairport,A[i].departure,A[i].destination,A[i].flightNum,A[i].planeNum,A[i].month,A[i].day,A[i].hour,A[i].min,A[i].nexthour,A[i].nextmin);
            price=A[i].price;
